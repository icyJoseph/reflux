...
import { Provider, useConnect, useNotify } from "../";
 ...
 */
const LabeledCountView = ({ count, label }) => {
...
const ConnectedInput = () => {
  const { tags } = useConnect(tagsReducer, ["a", "b"], tagSelector);
...
const ConnectedLabeledCountView = ({ spy }) => {
   ...
   *
   */
  const { count } = useConnect(reducer, { count: 0 }, selector(spy));
  return <LabeledCountView label={CONNECTED_BRANCH} count={count} />;
};
...
const ConnectedLabeledCountViewWithOwnState = ({ spy }) => {
  ...
  return (
    <>
      <LabeledCountView label={CONNECTED_BRANCH} tick={tick} count={count} />
